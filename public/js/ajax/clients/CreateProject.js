...
BlueBox.compose(
	...
	"ajax.clients.CreateProject", function(_class, _proto, _super) {
	...
	//get the id and the project name
	var recordID = this.getAJAX().createModeGetNewRecordID();
	var projectName = this.getAJAX().getClient().getInput("name").getValue();

	//clear the form. We don't want them to create the same project twice
	this.getAJAX().getClient().clearForm();
	...
	detailsRow1.html('<h2>Cool! You created a project: "'+projectName+'"</h2>');


	var detailsRow2 = HTMLHelper.i().row();
	detailsRow2.css("text-align","center");
	detailsRow2.css("margin-top","20px");
	detailsRow2.html('<h3><a href="'+window["_APP_BASE"]+'/user/edit-project/'+recordID+'">Edit your project</a>, or <a href="'+window["_APP_BASE"]+'/user/projects">See all your projects</a></h3>');
